...
web/tests/                models, decoders and transport, reducer and effects, component
                          rendering, and a wired-up integration pass
```

## Tests

582 tests, `npm test`. Most are unit tests against one module; the five suites under
`tests/integration` are different in kind, and it is worth saying what they buy. The dashboard
workspace carries its own 98 under `npm run test:web`, against jsdom rather than node.
